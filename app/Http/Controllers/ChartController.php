...
{   public function requested(Request $request){
    ...
    switch($mainoptionvalue){
        ...
        break;
        case 5:
        // $oldest = Spend::orderBy('created_at','asc')->pluck('created_at')->first();
        // $newest = Spend::orderBy('created_at','desc')->pluck('created_at')->first();
        // $oldests = $oldest->format('Y');
        // $newests = $newest->format('Y');
        // $years=  range(   $oldests ,  $newests);
        // $years['2017'] = '27';
        //     dd($years);

        
        $querys = DB::table('spends')
        ->select(
        ...
        foreach($querys as $value){
            ...
            array_push($amounts,$value->sum);
        }
        
        return view('admin.allyearspenchart')->with('years',json_encode($years))->with('amounts',json_encode($amounts));
